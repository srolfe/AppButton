...
- (void)drawSector:(SAMultisectorSector *)sector atPosition:(NSUInteger)position{
    ...
    CGContextSetLineWidth(context, 3.0);//saCircleLineWidth);
    
    
    UIColor *circleColor = sector.color;
    UIColor *endCircleColor = [UIColor colorWithHue:0.58 saturation:0.77 brightness:0.97 alpha:1];//[UIColor colorWithHue:0.58 saturation:0.66 brightness:0.89 alpha:1];//[UIColor colorWithHue:0 saturation:0 brightness:0.84 alpha:1];//[UIColor colorWithHue:0.51 saturation:1 brightness:0.99 alpha:1];//[UIColor colorWithHue:0.67 saturation:0.77 brightness:0.95 alpha:1];//[[UIColor grayColor] colorWithAlphaComponent:0.1];
	UIColor *startCircleColor = endCircleColor;
    ...
    CGContextAddArc(context, x, y, r, drawInf.circleLineAngle, drawInf.circleOffsetAngle, 0);
    ...
    //text on markers
    NSString *markerStrTemplate = [@"%.0f" stringByReplacingOccurrencesOfString:@"0" withString:[NSString stringWithFormat:@"%i", (int)self.numbersAfterPoint]];
...
- (SASectorDrawingInformation) sectorToDrawInf:(SAMultisectorSector *)sector position:(NSInteger)position{
    ...
    drawInf.radius = 90;//self.sectorsRadius * position;
	...
	drawInf.endMarkerRadius = drawInf.startMarkerRadius;
...
- (void) drawString:(NSString *)s withFont:(UIFont *)font color:(UIColor *)color withCenter:(CGPoint)center{
	/*CGSize a = [s sizeWithAttributes:@{NSFontAttributeName: font}];
	CGSize b = CGSizeMake(ceilf(a.width), ceilf(a.height));
	
    CGSize size = b;//[s sizeWithFont:font];
